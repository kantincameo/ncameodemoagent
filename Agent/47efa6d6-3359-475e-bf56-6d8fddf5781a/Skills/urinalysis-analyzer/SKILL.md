...
description: Analyzes a human-uploaded urinalysis report (PDF or screenshot) strictly against the bundled BloomCell_Urinalysis.xlsx ground-truth sheet and produces the fixed 8-object JSON output (Internal Notes, Understanding Your Results, Priority Actions - Next 30 Days, Long-Term Strategy). Use this skill whenever the user asks to run the "Urine analysis agent" / "Urinalysis agent", analyze a urine/urinalysis PDF or screenshot for the LEGO-piece pipeline, or generate provider/patient notes and 30-day/long-term plans from a urinalysis report. NO internet access, no external medical knowledge, and no model prior knowledge may be used for clinical content — every clinical statement must trace back to a row in the bundled Excel sheet. If the user supplies an updated version of BloomCell_Urinalysis.xlsx, replace the file in references/ with the new one and always use the latest file present there.
---

# Urinalysis Analyzer

Generates the "Urine Analysis" LEGO piece for the NCAMEO multi-agent health-report pipeline. Takes one human-selected urinalysis report file (PDF or screenshot) as input, and the bundled `references/BloomCell_Urinalysis.xlsx` sheet as the ONLY source of clinical rules, language, and ground truth. Produces exactly 8 JSON objects for human-in-the-loop review.

## Hard rules — read first

1. **Zero internet / zero external knowledge.** Never call web_search, never fetch a URL, never fill in a marker's interpretation, severity, or recommendation from general medical knowledge. If a marker found in the report is NOT present in `references/BloomCell_Urinalysis.xlsx`, do not invent content for it — list it in a `"skipped_markers"` note to the human instead of guessing (see Step 5).
...
3. **"Latest file always."** Before every run, list `references/` and use whichever `.xlsx` file is there. If the user gives you an updated sheet, overwrite the old one (same filename, or tell the user to keep the filename `BloomCell_Urinalysis.xlsx`) — never keep two versions around, never fall back to a cached/older copy.
...
Run `scripts/read_markers.py` to load every row of `references/BloomCell_Urinalysis.xlsx` into a clean structure: `{marker_name, internal_notes, explanation, plan_30_day, long_term_plan}`. This gives you the exact list of marker names the sheet recognizes (Color, Clarity/Turbidity, Specific Gravity, pH, Protein, Glucose, Ketones, Blood (Hemoglobin), Leukocyte Esterase, Nitrites, Bilirubin, Urobilinogen, RBCs, WBCs, Epithelial Cells, Casts, Crystals, Bacteria/Yeast, Mucus) — use these as your match targets, don't rely on memory of the sheet.

### Step 2 — Extract the report text
Never eyeball a PDF for numbers/findings. Run `scripts/extract_report.py <path-to-report>`:
- For a PDF, it runs `pdftotext -layout` and prints the raw extracted text.
- For an image/screenshot, view it directly (native vision) and transcribe every marker name + reading/flag (dipstick result, microscopic count, qualitative note) you see, verbatim, before doing anything else.
...
2. If no exact match, try a normalized match (strip units/parentheses, common abbreviation aliases e.g. "RBC" ↔ "Red Blood Cells (RBCs)", "WBC" ↔ "White Blood Cells (WBCs)", "SG" ↔ "Specific Gravity", "Leuk Est" ↔ "Leukocyte Esterase").
...
Only markers that are flagged abnormal in the report (positive/negative flips from normal, out-of-range dipstick or microscopic counts, or explicitly called out by the report) are candidates for detailed synthesis. Several rows in this sheet note that a finding is "provider-interpreted" rather than habit-driven (e.g. Leukocyte Esterase, Nitrites, Bilirubin, Casts, Bacteria/Yeast) — when synthesizing the 30-Day and Long-Term notes for those markers, keep to what the sheet's own plan columns say rather than inventing habit-based advice the sheet doesn't support. Normal-range markers generally don't need individual mention unless the report itself highlights them.
...
- `referencesource`: exact source — for highlight text this is the report file name + page/line; for note text this is `"BloomCell_Urinalysis.xlsx - Urinalysis Markers sheet - Row N, Marker: <name>"` (list every row used if more than one marker feeds a note)
...
Two conventions for `date` were mentioned when the sibling bloodwork-analyzer skill was defined: `MM/DD/YY HH:MM AM/PM` and UTC ISO 8601 (`2026-06-24T14:20:00Z`). This skill uses **UTC ISO 8601** for consistency with that skill and the actual JSON template — flag to the human if a different format is actually wanted so the script can be adjusted.
